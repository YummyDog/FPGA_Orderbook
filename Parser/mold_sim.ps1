<#
    ...
    Self-contained runner for the MoldUDP64 parser testbench. Does not depend
    on any other script and does not touch existing files.

    Compiles only what mold_parser needs:
        eth_parser_pkg.vhd    (package only - eth_parser NOT instantiated)
        ipv4_parser_pkg.vhd   (package only - ipv4_parser NOT instantiated)
        udp_parser_pkg.vhd    (package only - udp_parser NOT instantiated)
        mold_parser_pkg.vhd
        mold_parser.vhd

    Usage:
        powershell -ExecutionPolicy Bypass -File .\mold_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\mold_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\mold_sim.ps1 -Clean
#>
...
$Module   = "test_mold_parser"
$Toplevel = "mold_parser"
...
$BuildDir = Join-Path $Root "sim_build_mold"
$Results  = Join-Path $Root "results_mold.xml"
