<#
    ...
    Self-contained runner for the ITCH parser testbench. Does not depend on
    any other script and does not touch existing files.

    Compiles only what itch_parser needs. The four upstream parsers are NOT
    instantiated - only their packages are used, for the field-bus offsets.

    Usage:
        powershell -ExecutionPolicy Bypass -File .\itch_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\itch_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\itch_sim.ps1 -Clean
#>
...
$BuildDir = Join-Path $Root "sim_build_itch"
$Results  = Join-Path $Root "results_itch.xml"
