<#
    threeparserpipline_sim.ps1

    Self-contained runner for the three-stage pipeline testbench:
        eth_parser -> ipv4_parser -> udp_parser

    Does not depend on any other script and does not touch existing files.

    Usage:
        powershell -ExecutionPolicy Bypass -File .\threeparserpipline_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\threeparserpipline_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\threeparserpipline_sim.ps1 -Clean
#>
...
$Module   = "test_threeparserpipline"
$Toplevel = "threeparserpipline"
...
# Compile order matters: each package before the entity that uses it, and all
# three parsers before the wrapper that instantiates them.
$Sources = @(
    "eth_parser_pkg.vhd",
    "eth_parser.vhd",
    "ipv4_parser_pkg.vhd",
...
# Separate build dir and results file, so this cannot collide with the
# per-module runs in the same folder.
$BuildDir = Join-Path $Root "sim_build_pipeline"
$Results  = Join-Path $Root "results_pipeline.xml"
...
Write-Host "toplevel : $Toplevel  (eth -> ipv4 -> udp, chained)"
