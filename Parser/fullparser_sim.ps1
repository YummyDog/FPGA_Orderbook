<#
    ...
    Self-contained runner for the five-stage pipeline testbench:
    ...
    Usage:
        powershell -ExecutionPolicy Bypass -File .\fullparser_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\fullparser_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\fullparser_sim.ps1 -Clean
        powershell -ExecutionPolicy Bypass -File .\fullparser_sim.ps1 -Test test_double_completion
#>
...
# five parsers before the wrapper that instantiates them.
$Sources = @(
    "eth_parser_pkg.vhd",
    "eth_parser.vhd",
    "ipv4_parser_pkg.vhd",
...
$BuildDir = Join-Path $Root "sim_build_fullparser"
$Results  = Join-Path $Root "results_fullparser.xml"
...
Write-Host "toplevel : $Toplevel  (eth -> ipv4 -> udp -> mold -> itch)"
