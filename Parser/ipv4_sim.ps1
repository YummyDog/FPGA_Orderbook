<#
    ...
    Self-contained runner for the IPv4 parser testbench. Does not depend on
    run_sim.ps1 and does not touch any existing file.

    Compiles only what ipv4_parser needs:
        eth_parser_pkg.vhd   (for C_ETH_FIELDS_W - package only, eth_parser
                              itself is NOT instantiated)
        ipv4_parser_pkg.vhd
        ipv4_parser.vhd

    Usage:
        powershell -ExecutionPolicy Bypass -File .\ipv4_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\ipv4_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\ipv4_sim.ps1 -Clean
#>
...
$Module   = "test_ipv4_parser"
$Toplevel = "ipv4_parser"
...
# Ethernet run in the same folder.
$BuildDir = Join-Path $Root "sim_build_ipv4"
$Results  = Join-Path $Root "results_ipv4.xml"
