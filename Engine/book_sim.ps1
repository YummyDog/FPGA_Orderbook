<#
    ...
    Self-contained runner for the book_input_stage testbench. Does not depend
    ...
    Note the parser packages are NOT compiled: this module decodes raw ITCH
    message bytes itself and does not reference itch_parser_pkg.

    Usage:
        powershell -ExecutionPolicy Bypass -File .\book_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\book_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\book_sim.ps1 -Clean
        powershell -ExecutionPolicy Bypass -File .\book_sim.ps1 -Test test_executed_with_price
#>
...
$Module   = "test_book_input_stage"
$Toplevel = "book_input_stage"
...
if (-not (Test-Path (Join-Path $Root "book_model.py"))) {
    throw "Missing reference model: $(Join-Path $Root "book_model.py")"
}

# Separate build dir and results file, so this cannot collide with the other
# per-module runs in the same folder.
$BuildDir = Join-Path $Root "sim_build_book"
$Results  = Join-Path $Root "results_book.xml"
...
# G_ORDER_BOOK_ID must match book_model.DEFAULT_BOOK_ID. Without it the DUT
# filters on order book 0 and discards every message the testbench sends,
# which shows up as m_tvalid never asserting.
$RunArgs = @("-e", $Toplevel, "-gG_ORDER_BOOK_ID=$BookId", "--no-save", "-r", "--load", $VhpiLib)
