<#
    ...
    Self-contained runner for the order_book insertion testbench.
    ...
    Toplevel is order_book_top, not order_book: the engine exposes its memory
    ports rather than owning the RAM, so it cannot elaborate on its own. The
    testbench reads table contents out of the real RAM through the hierarchy.

    Usage:
        powershell -ExecutionPolicy Bypass -File .\order_book_sim.ps1
        powershell -ExecutionPolicy Bypass -File .\order_book_sim.ps1 -Waves
        powershell -ExecutionPolicy Bypass -File .\order_book_sim.ps1 -Clean
#>
...
$BuildDir = Join-Path $Root "sim_build_order_book"
$Results  = Join-Path $Root "results_order_book.xml"
...
Write-Host "toplevel : $Toplevel  (order_book + real ram_array)"
