...
-- fcs_msg_extend_sync
--
-- Single-clock version of fcs_msg_extend: no CDC, the FCS result and the
-- message count are already in this clock domain.
--
-- Stretches crc_complete / fcs_true / fcs_false to msg_count cycles. The FCS
-- result and msg_count_valid are paired in arrival order, one of each per
...
architecture rtl of fcs_msg_extend_sync is
  ...
  begin
    if rising_edge(clk) then
      ...
      if res_v = '1' then
        ...
        h_rv := crc_complete; h_rt := fcs_true; h_rf := fcs_false;
      end if;
      ...
      if res_v = '1' then
        if load then
          res_v <= crc_complete; res_t <= fcs_true; res_f <= fcs_false;
        elsif crc_complete = '1' then
          err_r <= '1';
        end if;
      elsif not load then
        res_v <= crc_complete; res_t <= fcs_true; res_f <= fcs_false;
